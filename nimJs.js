...
function computersTurn(){
	...
	if (isSumZero(first, second, third)){
		//random strategy
		pcRandomMove(nums);
	} else{
		//winning strategy
		pcWinningMove(first, second, third, nums);
	}
...
//calculates the nim sum of the piles if the sum is 0 returns true
function isSumZero(first, second, third){
	var xor = first^second^third;
	var isZero = false;
	if (xor == 0){
		isZero = true;
	}
...
//nim's sum!=0 and pc plays with winning strategy
function pcWinningMove(first, second, third, nums){
	var xor = first^second^third;
	var letters = ["a", "b", "c"];
	var binaryXor = xor.toString(2);
	var msb = Math.pow(2, binaryXor.length-1);
	for (var i = 0; i < 3; i++) {
		if((nums[i] & msb) != 0){
			letter = letters[i];
			numberToChange = nums[i];
			nums.splice(i, 1);
			break;
		}
	}

	var newNum = nums[0] ^ nums[1];
	setNewNumber(newNum, 0);
}

//nim's sum=0 and pc plays rndomly
