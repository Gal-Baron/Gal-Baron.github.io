...
if(isGameStart == "-1"){ //calculate random pile sizes only at the start.
	...
	var b = (Math.floor(Math.pow(goldenRatio, 2) * random)).toString();
...
// sets Wythoff's array of length accordint to random
for (var i = 0; i < random; i++) {
	cur = Math.floor(goldenRatio * i);
	winningDict[i] = cur;
	cur = Math.floor(Math.pow(goldenRatio, 2) * i);
	winningDictSqr[i] = cur;
}
...
function pcWinningMove(first, second) {
	if (first == second) {
		letter = "c";
		...
		while (n1 == 100 && n2 == 100 || bothPiles){
			// finds position of piles a and b in the Wythoff's array
			for (var i = 0; i < winningDictSqr.length; i++) {
				val = Math.floor(goldenRatio * i);
				if (val == first - numToTake) n1 = i;
				if (val == second - numToTake) n2 = i;
				val = Math.floor(Math.pow(goldenRatio, 2) * i);
				if (val == first - numToTake) n1 = i;
				if (val == second - numToTake) n2 = i;
			}
			...
			if (!bothPiles) {
				if (n1 < n2) {
					letter = "b";
					if (winningDict[n1] == first) {
						numToTake = second - winningDictSqr[n1];
					} else {
						numToTake = second - winningDict[n1];
					}
					if (numToTake > 0) {
						setNewNumber(first, second, second, numToTake);
						break;
					}					
				} else if (n2 < n1) {
					letter = "a";
					if (winningDict[n2] == second) {
						numToTake = first - winningDictSqr[n2];
					} else {
						numToTake = first - winningDict[n2];
					}
					if (numToTake > 0) {
						setNewNumber(first, second, first, numToTake);
						break;
					}
				}
			...
			// if none of the numbers were found in the array, the computer takes from c
			if (n1 != 100 && n1 == n2) {
				letter = "c";
				setNewNumber(first, second, first, numToTake);
				break;
